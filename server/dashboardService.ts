...
function valueRank(rows: ResultRow[], territoryId: string) {
...
function average(rows: ResultRow[]) {
...
function buildDetails(
  ...
  detailResults: Array<ResultRow & { componente_id?: string }>,
  primary: TerritoryRow,
  ...
  const resultByComponent = new Map<string, Array<ResultRow & { componente_id?: string }>>()
  ...
  function descend(component: ComponentRow, path: number[]): DetailRow {
    ...
    return {
      ...
      regionalRank: valueRank(peerResults, primary.id) ? `${valueRank(peerResults, primary.id)}º` : '—',
    ...
    try {
      ...
      const territoryResult = await client.query<TerritoryRow>(`
        select t.id, t.codigo, t.sigla, t.nome, t.tipo, t.parent_id,
          parent.codigo as parent_codigo, parent.nome as parent_nome
        from territorio t
        left join territorio parent on parent.id = t.parent_id
      ...
      const allResultsQuery = await client.query<ResultRow>(`
        ...
        order by rr.ano_referencia, rr.posicao nulls last
      `, [edition.id, metric.id])
      ...
      const selectedRegion = regions.find((row) => row.codigo === values.region)
        ?? regions.find((row) => row.id === primary.parent_id)
      ...
      const regionalSeries = years.map((year) => {
        const officialRegion = kind === 'ibid'
          ? allResults.find((row) => row.tipo === 'REGIAO' && row.codigo === selectedRegion?.codigo && row.ano_referencia === year)
          : undefined
        if (officialRegion?.nota !== null && officialRegion?.nota !== undefined) return officialRegion.nota
        return average(entityResults.filter((row) => row.ano_referencia === year && peerIds.has(row.id)))
      })
      const allChartValues = [...seriesByTerritory(primary), ...seriesByTerritory(comparison), ...regionalSeries]
      ...
      if (structureId) {
        ...
        const detailResult = await client.query<ResultRow & { componente_id: string }>(`
          select rr.componente_id, t.id, t.codigo, t.sigla, t.nome, t.tipo, t.parent_id,
            parent.codigo as parent_codigo, parent.nome as parent_nome,
            rr.ano_referencia, rr.nota_normalizada::float8 as nota, rr.posicao
          from resultado_ranking rr
          join territorio t on t.id = rr.territorio_id
          ...
          where rr.edicao_id = $1 and rr.componente_id = any($2::uuid[])
            ...
            and (t.tipo = $4 or ($5::boolean and t.tipo = 'REGIAO'))
        `, [edition.id, componentIds, selectedYear, config.entityType, kind === 'ibid'])
      ...
      const filters: FilterDefinition[] = kind === 'clp-municipios'
        ...
        : [
            ...
            { id: 'region', label: 'Região', value: selectedRegion?.codigo ?? '', options: regionOptions },
            { id: 'year', label: 'Ano', value: String(selectedYear), options: years.slice().reverse().map((year) => ({ label: String(year), value: String(year) })) },
            { id: 'metric', label: 'Métrica', value: metric.codigo, options: metrics.map((item) => ({ label: titleForMetric(item, kind), value: item.codigo })) },
          ]
